...
public class SliderLoadValidationTests
{
    public static IEnumerable<object[]> MalformedStates
    {
        get
        {
            ...
            yield return Case("slide node count parent mismatch", s => s.NNodes![0] = 3);
            yield return Case("null slide node arrays", s => s.Slides![0].NodeArrays = null);
            yield return Case("wrong slide node array count", s => s.Slides![0].NodeArrays = System.Array.Empty<double[]>());
            ...
            yield return Case("wrong slide weight count", s => s.Slides![0].Weights = System.Array.Empty<double[]>());
            ...
            yield return Case("short diff matrix sizes", s => s.Slides![0].DiffMatrixSizes = System.Array.Empty<int[]>());
            yield return Case("null diff matrix size row", s => s.Slides![0].DiffMatrixSizes = new int[][] { null! });
            ...
            yield return Case("short slide diff matrices", s => s.Slides![0].DiffMatrices = System.Array.Empty<double[]>());
