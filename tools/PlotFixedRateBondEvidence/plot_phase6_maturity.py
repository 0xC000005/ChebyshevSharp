#!/usr/bin/env python3
"""Generate fixed-rate bond maturity-sensitivity evidence plots.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate maturity schedule-sensitivity CSV and SVG evidence.")
    parser.add_argument(
        "--repo-root",
        type=Path,
...
def render_svg(boundary_date: str, points: list[ScanPoint]) -> str:
    ...
    parts.append(f'<text x="{left}" y="{height - 10}" font-family="Arial, sans-serif" font-size="11" fill="#6b7280">Generated by tools/PlotFixedRateBondEvidence/plot_maturity_sensitivity.py from --naive-maturity-scan-csv.</text>')
    parts.append("</svg>")
    return "\n".join(parts) + "\n"
